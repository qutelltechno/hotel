        ...
        <div class="content container-fluid">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <div class="mt-5">
                            <h4 class="card-title float-left mt-2">All room</h4>
                            <a href="{{ route('rooms.create') }}" class="btn btn-primary float-right veiwbutton">Add Room</a>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="table-responsive">
                                <table class="datatable table table-stripped table table-hover table-center mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td hidden class="id">{{ $room->id }}</td>
                                            {{-- <td hidden class="fileupload">{{ $room->fileupload }}</td> --}}
                                            <td>{{ $room->id }}</td>
                                            <td>
                                                <h2 class="table-avatar">
                                                {{-- <a href="profile.html" class="avatar avatar-sm mr-2">
                                                    <img class="avatar-img rounded-circle" src="{{ URL::to('/assets/upload/'.$room->fileupload) }}" alt="{{ $room->fileupload }}">
                                                </a> --}}
                                                <a href="profile.html">{{ $room->name }}<span>{{ $room->id }}</span></a>
                                                </h2>
                                            </td>
                                            <td>{{ $room->status }}</td>
                                            {{-- <td>{{ $room->room_type }}</td>
                                            <td>{{ $room->ac_non_ac }}</td>
                                            <td>{{ $room->food }}</td>
                                            <td>{{ $room->bed_count }}</td>
                                            <td>{{ $room->charges_for_cancellation }}</td>
                                            <td>{{ $room->rent }}</td>
